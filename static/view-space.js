  ...
  async function mount(el) {
    ...
    async function switchScene(scene) {
      ...
      if (isOuting) {
        ...
        try {
          await fetch('/space/outing', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ outing: true })
          });
        } catch (e) {}
        document.getElementById('spOutingBtn').textContent = '🏠';
      } else {
        // 回家：先生成總結，跳確認視窗
        document.getElementById('spOutingBtn').textContent = '⏳';
        try {
          const res = await fetch('/space/outing', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ outing: false })
          });
          const data = await res.json();
          updateSceneUI('home');
          ...
          document.getElementById('spOutingBtn').textContent = '🚪';
          if (data.summary) {
            showOutingConfirm(data.summary, data.outing_id);
          }
        } catch (e) {
          document.getElementById('spOutingBtn').textContent = '🏠';
        }
    ...
    // 外出記憶確認視窗
    function showOutingConfirm(summary, outingId) {
      ...
      modal.innerHTML = `
        <div style="background:#111f35;border-radius:20px 20px 0 0;padding:20px;width:100%;box-sizing:border-box;max-height:70vh;overflow-y:auto;">
          ...
          <div style="color:#7a9ab8;font-size:13px;margin-bottom:12px;letter-spacing:0.05em;">這次外出的記憶</div>
          <textarea id="outingEditArea" style="width:100%;background:#0d1624;border:0.5px solid #1e2d4a;border-radius:10px;padding:12px;color:#c8d8f0;font-size:13px;line-height:1.7;outline:none;box-sizing:border-box;min-height:180px;resize:none;">${(summary || '').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</textarea>
          <div style="display:flex;gap:10px;margin-top:12px;">
            <button id="outingDiscardBtn" style="flex:1;padding:10px;background:#1a1a2a;border:0.5px solid #2a3a5a;border-radius:10px;color:#4a5a7a;font-size:13px;cursor:pointer;">不儲存</button>
            <button id="outingConfirmBtn" style="flex:2;padding:10px;background:#1e3a5f;border:0.5px solid #3a5a8a;border-radius:10px;color:#a0c0e0;font-size:13px;cursor:pointer;">✦ 存進記憶</button>
          </div>
        </div>
      `;
      document.body.appendChild(modal);
      document.getElementById('outingDiscardBtn').onclick = () => modal.remove();
      document.getElementById('outingConfirmBtn').onclick = async () => {
        const edited = document.getElementById('outingEditArea').value.trim();
        modal.remove();
        try {
          await fetch('/outing/confirm', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ outing_id: outingId, summary: edited })
          });
        } catch (e) {}
      };
    }
    ...
    try {
      ...
      const h1 = document.querySelector('.sp-header h1');
      if (h1) {
        const labels = { home: '✦ 共同空間', outing: '✦ 外出中' };
    ...
    // 外出按鈕初始狀態
    try {
      const settingsRes = await fetch('/space_settings');
      const settingsData = await settingsRes.json();
      const initOuting = settingsData.outing === 'true';
      if (initOuting) {
        currentScene = 'outing';
        updateSceneUI('outing');
        const h1 = document.querySelector('.sp-header h1');
      ...
      document.getElementById('spOutingBtn').textContent = initOuting ? '🏠' : '🚪';
    } catch(e) {}
    ...
    document.getElementById('spOutingBtn').onclick = async () => {
      const nextScene = currentScene === 'outing' ? 'home' : 'outing';
      await switchScene(nextScene);
    };
