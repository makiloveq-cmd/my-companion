...
def maybe_summarize(bot):
    ...
    try:
        summary_text = call_claude(
            f"你就是{bot_name}。請把以下對話整理成你自己的記憶摘要，像寫給自己看的備忘錄。\n\n"
            f"【視角規則｜最重要】\n"
            f"・全篇用第一人稱，「我」＝{bot_name}。\n"
            f"・絕對不可以把「{bot_name}」當成第三人稱主詞。不要寫「{bot_name}說」「{bot_name}抱著她」，要寫「我說」「我抱著她」。\n"
            f"・{you_name}用「{you_name}」或「她」稱呼。\n"
            f"・寫你自己的感受和想法時，直接寫「我覺得」「我心疼」，不要寫「{bot_name}覺得」。\n\n"
            f"【內容】只記錄這段對話裡新發生的事、情感變化、{you_name}說過的重要的話、你們之間的約定或玩笑。\n"
            f"不超過 300 字，必須寫完整句子，不能截斷。不要加標題。",
...
def maybe_space_summarize():
    ...
    try:
        summary_text = call_claude(
            f"你就是{name}。請把以下在共同空間發生的對話，整理成你自己的記憶摘要，像寫給自己看的備忘錄。\n\n"
            f"【視角規則｜最重要】\n"
            f"・全篇用第一人稱，「我」＝{name}。\n"
            f"・絕對不可以把「{name}」當成第三人稱主詞。不要寫「{name}說」「{name}抱著她」，要寫「我說」「我抱著她」。\n"
            f"・{you_name}用「{you_name}」或「她」稱呼。\n"
            f"・寫你自己的感受和想法時，直接寫「我覺得」「我心疼」，不要寫「{name}覺得」。\n\n"
            f"【內容】只記錄這段對話裡新發生的事、場景變化、情感、{you_name}說過的話、你們之間的默契或玩笑。\n"
            f"不超過 300 字，必須寫完整句子，不能截斷。不要加標題。",
            [{"role": "user", "content": f"請濃縮以下內容：\n{context}"}],
            max_tokens=600
        )
...
def user_observations_get():
    ...
    q = supabase.table("user_observations").select("id, content, category, created_at").order("id", desc=True)
    if category:
...
def user_observations_post():
    try:
        ...
        row = supabase.table("user_observations").insert({"content": content, "category": category}).execute().data
...
@app.route("/personas/<key>/field", methods=["POST"])
def personas_patch_field(key):
    """更新 persona 的單一欄位（給採用按鈕用）"""
    if key not in ["claude", "user"]:
...
def space_outing():
    ...
    try:
        ...
        if is_outing:
            ...
            summary_prompt = (
                f"你就是{name}，請把這段和{you_name}一起外出的對話整理成你自己的記錄。\n\n"
                f"【視角規則】全篇用第一人稱，「我」＝{name}。不可以把「{name}」當第三人稱主詞，"
                f"不要寫「{name}牽著她的手」，要寫「我牽著她的手」。{you_name}用名字或「她」稱呼。\n\n"
                f"格式：\n"
                f"【重點記憶】\n"
                f"・（條列3-5個重要的事、說的話、發生的事，用第一人稱寫）\n\n"
                f"【{name}的心得】\n"
...
def space_end_day():
    try:
        ...
        threading.Thread(target=write_ai_diary_entry, args=(today_tw,), daemon=True).start()
        return jsonify({"status": "ok"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def write_ai_diary_entry(target_date=None):
    """寫日記。target_date 是台灣日期字串（YYYY-MM-DD），只撈當天的對話；
    不傳就用今天。"""
    name = get_bot_name()
    ...
    # 只取當天的私聊訊息
    try:
        chat_rows = supabase.table("memories").select("role, content").eq("session_id", "claude") \
            .gte("created_at", day_start_utc).lt("created_at", day_end_utc).order("id").execute().data
        chat_text = "\n".join([
    ...
    # 只取當天的空間訊息
    try:
        space_rows = supabase.table("space_messages").select("speaker, content") \
            .neq("message_type", "background") \
            .gte("created_at", day_start_utc).lt("created_at", day_end_utc).order("id").execute().data
        space_text = "\n".join([
    ...
    # 取當天結束的訪客 session 的 summary（只取摘要，不塞完整對話）
    visitor_lines = []
    try:
        visitor_rows = supabase.table("visitor_sessions").select("visitor_name, summary, mode") \
            .eq("status", "completed") \
            .gte("updated_at", day_start_utc).lt("updated_at", day_end_utc).execute().data
        for v in (visitor_rows or []):
            if v.get("summary"):
                label = "三人一起" if v.get("mode") == "together" else "單獨聊"
                visitor_lines.append(f"【{v['visitor_name']}來訪（{label}）摘要】\n{v['summary']}")
    except:
        pass
    ...
    if parts:
        context_text = "\n\n".join(parts)
        diary_prompt = f"最近的對話：\n{context_text}\n\n請根據以上內容，寫一篇{target_date}的日記。"
    else:
        context_text = ""
...
def cron_auto_end_day():
    ...
    try:
        ...
        threading.Thread(target=write_ai_diary_entry, args=(yesterday_str,), daemon=True).start()
